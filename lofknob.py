...
def lofknob(X, c_grid=None, k_grid=None):
    ...
    for c in c_grid:
        ...
        for k in k_grid:
            ...
            t_c_k = (m_c_k_out - m_c_k_in)/np.sqrt((v_c_k_out + v_c_k_in)/cn) if (m_c_k_out - m_c_k_in)*(v_c_k_out + v_c_k_in) > 1e-8 else 0
            
            t_c = np.append(t_c, t_c_k)
            
        # non-centrality parameter
        ncp_c = (v_c_out + v_c_in)/np.sqrt((v_c_out + v_c_in)/cn) if (v_c_out + v_c_in)*(v_c_out + v_c_in) > 1e-8 else 0
