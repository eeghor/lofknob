	...
	def tune(
		...
		for c in c_grid:
			...
			for k in k_grid:
				...
				if not label_counter.get(lofknob.OUTLIER_LABEL, None):
					print(f"LocalOutlierFactor found no outliers for contamination {c} and {k} neighbours")
					continue

				if not label_counter.get(lofknob.INLIER_LABEL, None):
					print(f"LocalOutlierFactor found no inliers for contamination {c} and {k} neighbours")
					continue
				...
				lof_scores = np.where(lof_scores > 1e+8, 1e+8, lof_scores)
				...
				if (
					...
					if ((sum_vars_this_k := out_var_lls_this_k + in_var_lls_this_k) > lofknob.VERY_SMALL):
						dist_score_this_k = dist_score_this_k / np.sqrt(sum_vars_this_k / expected_outlier_rows)
				else:
					dist_score_this_k = 0
			...
			if not out_mean_lls_all_ks:
				print(f"no outliers found for contamination {c} and any number of neighbours")
				continue
			...
			if (
				...
				if ((sum_mean_vars_over_ks := np.mean(out_var_lls_all_ks)
					+ np.mean(in_var_lls_all_ks)) > lofknob.VERY_SMALL):
			...
			if dist_scores_all_k:
				k_opt_this_c, opt_dist_score = max(
					sorted(dist_scores_all_k, key=lambda _: _[0]), 
					key=lambda x: x[1]
				)
			else:
				continue
			...
			if (degrees_of_freedom := 2 * (expected_outlier_rows - 1)) <= lofknob.VERY_SMALL:
				return None
			else:
				p_c = nct.cdf(
					x=opt_dist_score, df=degrees_of_freedom, nc=ncp_c, loc=0, scale=1
				)
				print(f"for this k={k_opt_this_c} opt_dist_score={opt_dist_score} df={degrees_of_freedom} prob score={p_c}")
		...
		print('these are all candidates:', candidates)
		if candidates:
			if return_scores:
				return sorted(
